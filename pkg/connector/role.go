...
func (r *roleResourceType) List(ctx context.Context, parentID *v2.ResourceId, opts rs.SyncOpAttrs) ([]*v2.Resource, *rs.SyncOpResults, error) {
...
func (r *roleResourceType) Grant(ctx context.Context, principal *v2.Resource, entitlement *v2.Entitlement) ([]*v2.Grant, annotations.Annotations, error) {
	...
	if principal.Id.ResourceType != resourceTypeUser.Id {
		...
		return nil, nil, fmt.Errorf("cloudamqp-connector: only users can be granted roles")
	}

	userId, roleId := principal.Id.Resource, entitlement.Resource.Id.Resource
	err := r.client.UpdateUserRole(ctx, userId, roleId)
	if err != nil {
		return nil, nil, fmt.Errorf("cloudamqp-connector: failed to update user role: %w", err)
	}

	newGrant := grant.NewGrant(entitlement.Resource, roleMember, principal.Id)
